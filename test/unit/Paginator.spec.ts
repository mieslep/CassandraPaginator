...
describe('Paginator', () => {
    ...
    it('correctly calculates total pages', () => {
        ...
        const paginator = new Paginator<number>(mockCacheManager, values, pageSize);
    ...
    it('ensures cache manager set method is called correctly', () => {
        const values = Array.from({ length: 104 }, (_, i) => i + 1);
        new Paginator<number>(mockCacheManager, values, pageSize);
    ...
    it('handles empty values array gracefully', () => {
        const paginator = new Paginator<number>(mockCacheManager, [], pageSize);
    ...
    it('behaves correctly when page size is larger than total items', () => {
        ...
        const paginator = new Paginator<number>(mockCacheManager, values, 10); // Page size of 10
    ...
    it('handles single-item scenario correctly', () => {
        const paginator = new Paginator<number>(mockCacheManager, [42], pageSize);
        expect(paginator.getTotalPages()).toBe(1);
